...
if($uinfo==false)
{
	...
	foreach($teachingPractice as $tp) {
        ...
        //# Keywords still to be implemented properly
		$template->pageData['mainBody'] .= "<tr class='alt'><td>Keywords:</td></tr>";
		$template->pageData['mainBody'] .= "<tr><td><i>Still to be implemented</i></td></tr>";
		if($tp['problemstatement']!= false){
			$template->pageData['mainBody'] .= "<tr class='alt'><td>Rationale statement:</td></tr>";
			$template->pageData['mainBody'] .= "<tr><td>{$tp['problemstatement']}</td></tr>";
		}
		if($tp['thisbundle']!= false) {
			$template->pageData['mainBody'] .= "<tr class='alt'><td>This Teaching Tip: </td></tr>";
			$template->pageData['mainBody'] .= "<tr><td>{$tp['thisbundle']}</td></tr>";
		}
		if($tp['wayitworks']!= false) {
			$template->pageData['mainBody'] .= "<tr class='alt'><td>What I did: </td></tr>";
		...
		if($tp['doesntwork']!= false) {
			$template->pageData['mainBody'] .= "<tr class='alt'><td>This doesn't work if:</td></tr>";
    ...
    $comments = array(); //# temp until db sorted
	...
	if($comments != false) {
		...
		foreach ($comments as $c) {
           ...
           $replies = array(); //# temp until DB sorted.
